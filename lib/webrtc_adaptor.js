...
export default function WebRTCAdaptor(initialValues) {
	class PeerStats {
		...
		get averageIncomingBitrate() {
			return Math.floor(
				(8 *
					(this.totalBytesReceivedCount -
						this.firstBytesReceivedCount)) /
					(this.currentTimestamp - this.startTime)
			);
		}
	...
	this.initPeerConnection = function (streamId, dataChannelMode) {
		if (thiz.remotePeerConnection[streamId] == null) {
			...
			if (!thiz.playStreamId.includes(streamId)) {
				if (thiz.localStream != null) {
					thiz.remotePeerConnection[streamId].addStream(
						thiz.localStream
					);
				}
			}
			thiz.remotePeerConnection[streamId].onicecandidate = function (
				event
			) {
			...
			if (dataChannelMode == 'publish') {
				...
				thiz.remotePeerConnection[streamId].ondatachannel = function (
					ev
				) {
				...
				if (thiz.remotePeerConnection[streamId].createDataChannel) {
					...
					thiz.remotePeerConnection[streamId].ondatachannel =
						function (ev) {
							thiz.initDataChannel(streamId, ev.channel);
						};
				} else {
			...
			thiz.remotePeerConnection[streamId].oniceconnectionstatechange =
				function (event) {
					var obj = {
						state: thiz.remotePeerConnection[streamId]
							.iceConnectionState,
						streamId: streamId
					};
					...
					if (!thiz.isPlayMode) {
						if (
							...
							thiz.changeBandwidth(thiz.bandwidth, streamId)
								.then(() => {
									console.log(
		...
		thiz.remotePeerConnection[streamId]
			...
			.then(function (response) {
				...
				for (var i = 0; i < length; i++) {
					thiz.addIceCandidate(
						streamId,
						thiz.iceCandidateList[streamId][i]
					);
				}
	...
	this.getStats = function (streamId) {
		thiz.remotePeerConnection[streamId].getStats(null).then((stats) => {
			...
			thiz.remotePeerConnectionStats[streamId].fractionLost =
				fractionLost;
			...
			thiz.callback(
				'updated_stats',
				thiz.remotePeerConnectionStats[streamId]
			);
		});
	};
